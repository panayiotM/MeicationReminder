        ...
        for (int i = 0; i < medicineArrayList.size(); i++) {
            for (int alarm = 0; alarm < medicineArrayList.get(i).getTime().split(",").length; alarm++) {
                ...
                if (timeInMills - currentTime >= 0 && timeInMills - currentTime < smallest) {
                    ...
                    refillLimit = 5;
                    medAmount = medicineArrayList.get(i).getMedCap() - medicineArrayList.get(i).getPillCount();
                    refillDate=medicineArrayList.get(i).getRefillTime();
                    //medIcon = medicineArrayList.get(i).getImage();
                }
